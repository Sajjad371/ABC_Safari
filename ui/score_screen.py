    ...
    def __init__(self, parent):
        ...
        if os.path.exists(p_bg):
            try:
                ...
                print(f"Error loading score background: {e}")
...
def show_score_screen(root, player_name="Explorer", score=0, stars=0, weak_letters=None, on_play_again=None, on_quit=None):
    ...
    if os.path.exists(p_fallback):
        try:
            pil_fb = Image.open(p_fallback).convert("RGBA")
            img_fb = ctk.CTkImage(light_image=pil_fb, size=(312, 332))
        except Exception: pass

    if img_fb:
        ctk.CTkLabel(media_panel, image=img_fb, text="", fg_color="transparent").place(relx=0.5, rely=0.5, anchor="center")
    else:
        ctk.CTkLabel(media_panel, text="[Dance GIF Box]", font=_font(28, "bold"), text_color="#FFFFFF").place(relx=0.5, rely=0.5, anchor="center")
